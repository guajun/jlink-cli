...
jlink-cli dll connect --device STM32H750VB --interface SWD --speed 4000 --dry-run
...
`dll connect` uses the direct J-Link DLL backend to open, configure, and connect a target session. It defaults to dry-run behavior and requires `--yes` before opening an exclusive J-Link session.
